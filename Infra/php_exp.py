...
def clear_log(url='', viewFile=''):
    ...
    while (requests.post(url=url, json=data, headers=header, verify=False).status_code != 200): 
        print(requests.post(url=url, json=data, headers=header, verify=False).status_code)
        pass
