...
h1 = {'xauthorization':"Hfal125kadhalssf"}

def verify(url):
    resp = requests.get(url, proxies=proxy, headers=h1, verify=False)
...
def register(url):
    ...
    resp = requests.get(url + guest_config_api, proxies=proxy, headers=h1, verify=False).json()['data']
    ...
    resp = requests.post(url + register_api, data=post_data, headers=h1, proxies=proxy, verify=False)
...
def login(url, email, password):
    ...
    resp = requests.post(url + login_api, data=data, headers=h1, proxies=proxy, verify=False)
    
    if resp.status_code == 200:
        auth_data = resp.json()['data']['auth_data']
        header = {'Authorization': auth_data, 'xauthorization':"Hfal125kadhalssf"}
